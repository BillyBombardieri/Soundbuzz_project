<?php 
    ...
    if( $_POST ){ 
        if(!empty($_POST['mail'])) {
            $r = $soundbuzz->prepare("SELECT id_user FROM user WHERE email = '$_POST[mail]'");
            $r->execute(); 
            if($r == NULL ){
                $inscrip .= '<div style="color:red;text-align:center;">Aucun compte n\'est associé à cet Email</div>';
            }
            if($r != NULL ) {
                ...
                $message="Merci de bien vouloir cliquer sur ce lien afin de saisir votre nouveau mot de passe : http://localhost:8080/reset_password.php?id_utilisateur=".$r;
                // Envoie du mail
