<?php 
    ...
    if( $_POST ){ 
        if(!empty($_POST['mail'])  && !empty($_POST['mdp']) && !empty($_POST['prenom']) && !empty($_POST['nom']) ) {
            $r = $soundbuzz->prepare("SELECT * FROM user WHERE email = '$_POST[mail]' ");
            $r->execute(); 
            if($r->rowCount() >= 1 ){
            ...
            if( $_POST['confirm_mdp'] == $_POST['mdp'] ) {
                ...
                if( empty($inscrip) ){ //Si ma variable $inscrip est vide
                    $soundbuzz->exec("INSERT INTO user(nom, prenom, mdp, email) VALUES('$_POST[nom]', '$_POST[prenom]', '$_POST[mdp]', '$_POST[mail]')  ");
