<?php 
    ...
    if( $_POST ){
        $r = $soundbuzz->query("SELECT * FROM user WHERE email = '$_POST[mail]' ");
        //Si il y a une correpondance d'un mail dans la table 'utilisateur', $r renverra '1' ligne de résultat
        if( $r->rowCount() >= 1){
            $utilisateur = $r->fetch(PDO::FETCH_ASSOC);
            if( password_verify( $_POST['mdp'], $utilisateur['mdp']) ){
                $_SESSION['user']['id_user'] = $utilisateur['id_user'];
                $_SESSION['user']['email'] = $utilisateur['email'];
                $_SESSION['user']['mdp'] = $utilisateur['mdp'];
                $_SESSION['user']['prenom'] = $utilisateur['prenom'];
                $_SESSION['user']['nom'] = $utilisateur['nom'];
                $_SESSION['user']['droit'] = $utilisateur['droit'];
                session_start();
                header('location:accueil.php');
            }
            else{
            ...
            $connex .= '<div style="color:red;text-align:center;">Email non valide</div>';
        }
    }
...
<body>
    ...
    <section>
        <h1>Sound<b>Buzz</b></h1><br>
        <p><b>Identifiez-vous</b>
        pour accéder à  vos musics préférées</p>
        <br><br>
        <?php echo $connex; ?>
        <br>
